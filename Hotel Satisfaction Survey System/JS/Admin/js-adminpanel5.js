...
document.addEventListener('DOMContentLoaded', function () {
    const saved = localStorage.getItem("roomSurveySection5"); // <-- use correct key for Room Quality
    ...
    const canvas = document.getElementById('overallSatisfactionChart'); // <-- canvas id must be roomQualityChart
    if (!canvas) {
        console.error('Canvas element with id "overallSatisfactionChart" not found.');
        return;
    }
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Overall Satisfaction Ratings',
                data: ratingCounts,
                backgroundColor: [
                    '#ff4d4d',
                    '#ff9933',
