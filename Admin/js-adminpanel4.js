document.addEventListener('DOMContentLoaded', function () {
    const saved = localStorage.getItem("roomSurveySection4"); // <-- use correct key for Room Quality
    ...
    const canvas = document.getElementById('valueforMoneyChart'); // <-- canvas id must be roomQualityChart
    if (!canvas) {
        console.error('Canvas element with id "valueforMoneyChart" not found.');
        return;
    }
